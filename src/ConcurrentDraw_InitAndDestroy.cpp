...
CD_API(sint) InitializeFileStreamEx (schar *FileName)
	...
	AS.channelLength = 8 * streamLength / (info.chans * info.freq * info.origres);
...
CD_API(sint) InitializeSpectrogramEx (uint FrameWidth, uint FrameHeight, uchar PaletteNumber, uchar SpectrogramMode)
	...
	AS.cdBMPInfo.cd_bmpinfo.header.biSize = sizeof (BITMAPINFOHEADER);
	AS.cdBMPInfo.cd_bmpinfo.header.biWidth = AS.sgFrameWidth;
	AS.cdBMPInfo.cd_bmpinfo.header.biHeight = AS.sgFrameHeight;
	AS.cdBMPInfo.cd_bmpinfo.header.biPlanes = 1;
	AS.cdBMPInfo.cd_bmpinfo.header.biBitCount = 8;
	AS.cdBMPInfo.cd_bmpinfo.header.biClrUsed = AS.cdBMPInfo.cd_bmpinfo.header.biClrImportant = CD_BMPINFO_COLORS_COUNT;

	FillPaletteEx (PaletteNumber);

	// Создание BITMAP
	if ((AS.sgBMP = CreateDIBSection (NULL, (BITMAPINFO *)&AS.cdBMPInfo, DIB_RGB_COLORS, (void **)&AS.sgBuffer, NULL, 0)) == NULL)
		return -4;
